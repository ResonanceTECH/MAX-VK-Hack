...
class MessageHandler(BaseHandler):
    ...
    def handle_send_to_teacher(self, user: Dict, max_user_id: int, text: str,
        ...
        if text.lower() in ['отмена', 'cancel', '/cancel']:
            logger.info(
                f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отмена_отправки_преподавателю")
        ...
        if teacher:
            logger.info(
                f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отправлено_сообщение_преподавателю_{teacher.get('fio', 'Unknown')}")
    ...
    def handle_send_to_student(self, user: Dict, max_user_id: int, text: str,
        ...
        if text.lower() in ['отмена', 'cancel', '/cancel']:
            logger.info(
                f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отмена_отправки_студенту")
            clear_state(max_user_id)
            self.show_main_menu(user, None, max_user_id, api)
            return
        ...
        if student:
            logger.info(
                f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отправлено_сообщение_студенту_{student.get('fio', 'Unknown')}")
    ...
    def handle_send_to_support(self, user: Dict, max_user_id: int, text: str,
        ...
        if text.lower() in ['отмена', 'cancel', '/cancel']:
            logger.info(
                f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отмена_отправки_в_поддержку")
            clear_state(max_user_id)
        ...
        if support_user:
            logger.info(
                f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отправлено_сообщение_в_поддержку_{support_user.get('fio', 'Unknown')}")
    ...
    def handle_broadcast_message(self, user: Dict, max_user_id: int, text: str,
        ...
        if text.lower() in ['отмена', 'cancel', '/cancel']:
            logger.info(
                f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отмена_рассылки")
            clear_state(max_user_id)
            self.show_main_menu(user, None, max_user_id, api)
            return
        ...
        if group:
            logger.info(
                f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=рассылка_группе_{group.get('name', 'Unknown')}")
    ...
    def handle_group_message(self, user: Dict, max_user_id: int, text: str,
        ...
        if text.lower() in ['отмена', 'cancel', '/cancel']:
            logger.info(
                f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отмена_отправки_от_группы")
            clear_state(max_user_id)
            self.show_main_menu(user, None, max_user_id, api)
            return
        ...
        if group and teacher:
            logger.info(
                f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отправлено_сообщение_от_группы_{group.get('name', 'Unknown')}_преподавателю_{teacher.get('fio', 'Unknown')}")
    ...
    def handle_send_to_student_student(self, user: Dict, max_user_id: int, text: str,
        ...
        if text.lower() in ['отмена', 'cancel', '/cancel']:
            logger.info(
                f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отмена_отправки_студенту_студенту")
            clear_state(max_user_id)
            self.show_main_menu(user, None, max_user_id, api)
            return
    ...
    def handle_broadcast_headmen(self, user: Dict, max_user_id: int, text: str,
        ...
        if text.lower() in ['отмена', 'cancel', '/cancel']:
            logger.info(
                f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отмена_рассылки_старостам")
            clear_state(max_user_id)
            self.show_main_menu(user, None, max_user_id, api)
            return
    ...
    def handle_admin_state(self, user: Dict, max_user_id: int, text: str, state: str,
        ...
        if state == 'admin_student_add':
            ...
            if text.lower() in ['отмена', 'cancel', '/cancel']:
                logger.info(
                    f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отмена_рассылки_студентам")
                clear_state(max_user_id)
                from handlers.callback import CallbackHandler
                callback_handler = CallbackHandler()
            ...
            if text.lower() in ['отмена', 'cancel', '/cancel']:
                logger.info(
                    f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отмена_рассылки_преподавателям")
                clear_state(max_user_id)
                from handlers.callback import CallbackHandler
                callback_handler = CallbackHandler()
                callback_handler.show_admin_broadcasts_menu(user, max_user_id, api)
    ...
    def handle_edit_schedule(self, user: Dict, max_user_id: int, text: str, api, message_id: str):
        ...
        if text.lower() in ['отмена', 'cancel', '/cancel']:
            logger.info(
                f"[USER] user_id={max_user_id}, first_name={user.get('fio', 'Unknown')}, action=отмена_редактирования_расписания")
            clear_state(max_user_id)
            from handlers.callback import CallbackHandler
            callback_handler = CallbackHandler()
